...
namespace HastaKalistaBaby
{
    class Damage
    {
        public static float GetQDmg(Obj_AI_Base target)
        {
            ...
            return (float)ObjectManager.Player.CalcDamage(target, DamageType.Physical, dmg);
        }
        ...
        public static float GetWDmg(Obj_AI_Base target)
        {
            ...
            * target.MaxHealth;
            return (float)Program.Player.CalcDamage(target, DamageType.Magical, dmg);
        ...
        public static float GetEdamage(Obj_AI_Base target)
        {
            if (target.GetBuffCount("kalistaexpungemarker") > 0)
            {
                ...
                return
                (float)
                    Program.Player.CalcDamage(target, DamageType.Physical, dmg + target.FlatHPRegenMod);
            }
            return 0;
