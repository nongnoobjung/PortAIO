...
namespace ARAMDetFull
{
    class ARAMTargetSelector
    {
        ...
        {
            List<AIHeroClient> targetable_ones = HeroManager.Enemies.Where(ob => ob != null && !IsInvulnerable(ob) && !ob.IsDead && !ob.IsZombie && (ob.LSIsValidTarget((!calcInRadius) ? range : range + 90) || ob.LSIsValidTarget((!calcInRadius) ? range : range + 90, true, fromPlus))).ToList();
